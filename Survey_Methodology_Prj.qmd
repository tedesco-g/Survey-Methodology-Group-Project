...
hcmap("custom/europe", 
  ...
  hc_chart(
    width = 1200,  # Increase width for a broader view
    height = 850   # Increase height to avoid squeezing the map
  ) %>%
  hc_mapNavigation(
  ...
  hc_title(text = "Transgender Civil DC Support Levels in Europe (Zoomed Out)")
...
The Gradient Boosting Machine (GBM) model achieved an accuracy of 0.5262, performing worse than other models, likely due to overfitting and suboptimal parameter tuning.
...
normalize_predictions <- function(probs) {
  # Check for different possible input types
  if (is.null(probs)) {
    stop("Prediction input is NULL")
  }
  
  # Handle matrix with 'Yes'/'No' columns
  if (is.matrix(probs) && all(c("Yes", "No") %in% colnames(probs))) {
    return(probs[, "Yes"])
  }
  
  # Handle matrix with 2 columns
  if (is.matrix(probs) && ncol(probs) == 2) {
    return(probs[, 2])  # Assume second column is the positive class
  }
  
  # Handle matrix with multiple columns (likely multi-class)
  if (is.matrix(probs) && ncol(probs) > 2) {
    return(probs[, 1])  # Default to first column
  }
  
  # Handle vector of probabilities
  if (is.vector(probs) && is.numeric(probs)) {
    return(probs)
  }
  
  # If we've made it this far, we can't normalize
  stop(paste("Cannot normalize probabilities. ",
             "Unexpected type: ", class(probs), 
             ", Columns: ", 
             ifelse(is.matrix(probs), ncol(probs), "N/A"),
             ", Columns names: ", 
             ifelse(is.matrix(probs), 
                    paste(colnames(probs), collapse=", "), 
                    "N/A")))
}

# Comprehensive Ensemble Prediction Function
ensemble_predictions <- function(rf_probs, xgb_probs, gbm_probs, test_y) {
  # Normalize predictions
  rf_yes_probs <- normalize_predictions(rf_probs)
  xgb_yes_probs <- normalize_predictions(xgb_probs)
  gbm_yes_probs <- normalize_predictions(gbm_probs)
  
  # Ensure consistent length
  if (!(length(rf_yes_probs) == length(xgb_yes_probs) && 
        length(rf_yes_probs) == length(gbm_yes_probs))) {
    stop("Prediction vectors have different lengths")
  }
  ...
  ensemble_preds <- ifelse(ensemble_preds_prob > 0.5, "Yes", "No")
...
# Diagnostic Function
diagnose_predictions <- function(rf_probs, xgb_probs, gbm_probs) {
  # Random Forest Predictions
  cat("Random Forest Predictions:\n")
  cat("Class:", class(rf_probs), "\n")
  cat("Type:", typeof(rf_probs), "\n")
  cat("Dimensions:", 
      ifelse(is.matrix(rf_probs) || is.data.frame(rf_probs), 
             paste(nrow(rf_probs), "x", ncol(rf_probs)), 
             "N/A"), "\n")
  if (is.matrix(rf_probs) || is.data.frame(rf_probs)) {
    cat("Column names:", paste(colnames(rf_probs), collapse=", "), "\n")
  }
  cat("First few values:\n")
  print(head(rf_probs))
  
  # Repeat for XGB and GBM
  cat("\nXGBoost Predictions:\n")
  cat("Class:", class(xgb_probs), "\n")
  cat("Type:", typeof(xgb_probs), "\n")
  cat("Dimensions:", 
      ifelse(is.matrix(xgb_probs) || is.data.frame(xgb_probs), 
             paste(nrow(xgb_probs), "x", ncol(xgb_probs)), 
             "N/A"), "\n")
  if (is.matrix(xgb_probs) || is.data.frame(xgb_probs)) {
    cat("Column names:", paste(colnames(xgb_probs), collapse=", "), "\n")
  }
  cat("First few values:\n")
  print(head(xgb_probs))
  
  cat("\nGBM Predictions:\n")
  cat("Class:", class(gbm_probs), "\n")
  cat("Type:", typeof(gbm_probs), "\n")
  cat("Dimensions:", 
      ifelse(is.matrix(gbm_probs) || is.data.frame(gbm_probs), 
             paste(nrow(gbm_probs), "x", ncol(gbm_probs)), 
             "N/A"), "\n")
  if (is.matrix(gbm_probs) || is.data.frame(gbm_probs)) {
    cat("Column names:", paste(colnames(gbm_probs), collapse=", "), "\n")
  }
  cat("First few values:\n")
  print(head(gbm_probs))
}

# Run diagnostics
print("Diagnosing prediction formats...")
diagnose_predictions(rf_cv, xgb_accuracy, result)

# Attempt ensemble predictions
...
results <- data.frame(
  ...
  Accuracy = c(rf_accuracy, logit_accuracy, dt_accuracy, 
               xgb_accuracy,gbm_accuracy, ensemble_accuracy))
...
best_model_preds <- switch(tolower(best_model_name),
                          ...
                          "multinomial logistic" = logit_preds,
...
Asher & Lyric. (2023, August 8). Global Trans Rights Index: 203 countries ranked in 2023.[ https://www.asherfergusson.com/global-trans-rights-index/](https://www.asherfergusson.com/global-trans-rights-index/)

European Institute for Gender Equality. (n.d.). Culture.[ https://eige.europa.eu/gender-mainstreaming/policy-areas/culture?language_content_entity=en](https://eige.europa.eu/gender-mainstreaming/policy-areas/culture?language_content_entity=en)
...
TGEU - Transgender Europe. (2025, March 13). Homepage.[ https://tgeu.org/](https://tgeu.org/)
