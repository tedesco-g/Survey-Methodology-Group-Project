...
model_data <- final_data[, names(final_data) != "iso3c1"]
...
# Ensure train_data_filtered is loaded and valid
if (!exists("train_data_filtered")) {
    stop("Error: train_data_filtered does not exist!")
}

# Check for missing values
na_counts <- colSums(is.na(train_data_filtered))
if (any(na_counts > 0)) {
    cat("Warning: Missing values found! Fixing...\n")
    for (col in names(train_data_filtered)) {
        if (any(is.na(train_data_filtered[[col]]))) {
            if (is.numeric(train_data_filtered[[col]])) {
                train_data_filtered[[col]][is.na(train_data_filtered[[col]])] <- median(train_data_filtered[[col]], na.rm = TRUE)
            } else {
                train_data_filtered[[col]][is.na(train_data_filtered[[col]])] <- as.character(stats::na.omit(train_data_filtered[[col]]))[1]
            }
        }
    }
...
train_data_filtered$transgender_civil_dc <- as.factor(train_data_filtered$transgender_civil_dc)
...
basic_features <- c(
  "school_div_transgender.L", 
  "school_div_sexual_orientation.L",
  "school_div_intersex.L", 
  "age_edu"
)
...
gbm_model <- gbm(
  ...
  n.trees = 500,  # Higher trees for better learning
  interaction.depth = 5,  # More depth to capture interactions
  shrinkage = 0.005,  # Lower shrinkage for stable learning
  verbose = FALSE
)

# ✅ Make Predictions
gbm_preds_prob <- predict(gbm_model, newdata = test_x_selected, n.trees = 500, type = "response")

# ✅ Convert probabilities to class labels
gbm_preds_class <- apply(gbm_preds_prob, 1, which.max)
gbm_preds <- factor(levels(train_y_safe)[gbm_preds_class], levels = levels(train_y_safe))
...
# Convert models to probability outputs
rf_preds_prob <- predict(rf_cv, newdata = test_x, type = "prob")
xgb_preds_prob <- predict(xgb_model, newdata = xgb_test)
gbm_preds_prob <- predict(gbm_model, newdata = test_x, n.trees = best_iter, type = "response")

# Average probability predictions
ensemble_preds_prob <- (rf_preds_prob + xgb_preds_prob + gbm_preds_prob) / 3
ensemble_preds <- ifelse(ensemble_preds_prob > 0.5, "Yes", "No")
ensemble_accuracy <- mean(ensemble_preds == test_y)

print(paste("Ensemble Accuracy:", round(ensemble_accuracy, 4)))
```
...
results <- data.frame(
  Model = c("Random Forest", "Multinomial Logistic", "Decision Tree", 
            "XGBoost", "KNN", "Gradient Boosting", "Ensemble"),
  Accuracy = c(rf_accuracy, logit_accuracy, dt_accuracy, 
               xgb_accuracy, knn_accuracy, 
               gbm_accuracy, ensemble_accuracy)
)
...
The results indicate that Random Forest and Multinomial Logistic Regression achieved perfect accuracy (1.000), which is highly suspicious and suggests potential overfitting or data leakage. These models may have inadvertently learned patterns specific to the training data rather than generalizable decision rules. K-Nearest Neighbors (KNN) performed well with an accuracy of 0.721, followed by XGBoost at 0.629, and Decision Tree at 0.609, both of which offered moderate predictive power. Gradient Boosting (GBM) had a significantly lower accuracy of 0.526, possibly due to the known instability of the multinomial GBM implementation. The Ensemble Model, which combined multiple classifiers, failed to improve performance, achieving only 0.376, indicating that the individual models did not complement each other effectively. Given these results, KNN and XGBoost emerge as the most reliable models, balancing predictive accuracy without signs of overfitting, whereas Random Forest and Multinomial Logistic Regression require further validation to confirm their legitimacy.
